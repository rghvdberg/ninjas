/* (Auto-generated binary data file). */

#ifndef BINARY_NINJASARTWORK_HPP
#define BINARY_NINJASARTWORK_HPP

namespace NinjasArtwork
{
extern const char* backgroundData;
const unsigned int backgroundDataSize = 1339800;
const unsigned int backgroundWidth    = 1276;
const unsigned int backgroundHeight   = 350;

extern const char* floppy_offData;
const unsigned int floppy_offDataSize = 78912;
const unsigned int floppy_offWidth    = 274;
const unsigned int floppy_offHeight   = 96;

extern const char* floppy_onData;
const unsigned int floppy_onDataSize = 78912;
const unsigned int floppy_onWidth    = 274;
const unsigned int floppy_onHeight   = 96;

extern const char* rotary_adsrData;
const unsigned int rotary_adsrDataSize = 7744;
const unsigned int rotary_adsrWidth    = 44;
const unsigned int rotary_adsrHeight   = 44;

extern const char* rotary_slicesData;
const unsigned int rotary_slicesDataSize = 88752;
const unsigned int rotary_slicesWidth    = 43;
const unsigned int rotary_slicesHeight   = 688;

extern const char* sliderData;
const unsigned int sliderDataSize = 704;
const unsigned int sliderWidth    = 11;
const unsigned int sliderHeight   = 16;

extern const char* switch_offData;
const unsigned int switch_offDataSize = 2610;
const unsigned int switch_offWidth    = 30;
const unsigned int switch_offHeight   = 29;

extern const char* switch_onData;
const unsigned int switch_onDataSize = 2610;
const unsigned int switch_onWidth    = 30;
const unsigned int switch_onHeight   = 29;

const unsigned int lcd_left = 366;
const unsigned int lcd_right = 922;
const unsigned int lcd_top = 56;
const unsigned int lcd_bottom = 163;
const unsigned int lcd_center = lcd_bottom - lcd_top;

}

#endif // BINARY_NINJASARWORK_HPP

